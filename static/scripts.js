document.getElementById("simulate-btn").addEventListener("click", function() {
    ...
    function getPenaltyShootoutScore() {
        // Ensure that one team always wins by generating a higher score for the winner
        let team1Penalties = Math.floor(Math.random() * 5);
        let team2Penalties = Math.floor(Math.random() * 5);
        
        // If the penalty scores are equal, make sure one team wins
    ...
    function getLogoFilename(teamName) {
        const teamLogos = {
            ...
            // Add any other team mappings if necessary
        };
    ...
    // Champion display
    const champion = finalResult.winner;  // Correctly extract the winner's name from the final result
